...
function App() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.error(error);
      setMessage('❌ Upload failed! Apne server and .env check karein.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-gray-800 p-8">
      <h1 className="text-4xl font-bold mb-4">Drive Image Store</h1>
      <p className="text-gray-600 mb-8 text-center max-w-md">
        Aapke sabhi uploads is page ke niche grid mein dikhenge. 
      </p>
      ...
      <div className="flex flex-col items-center gap-6 bg-white p-8 rounded-xl shadow-sm border border-gray-100 w-full max-w-md">
        ...
        {/* Status Message */}
        {message && (
          <p className={`text-sm mt-2 font-medium ${message.includes('❌') ? 'text-red-500' : 'text-green-600'}`}>
            {message}
          </p>
        )}
      ...
      <ImageGrid refreshTrigger={refreshGrid} />
    </div>
  )
}
